...
BOOL						bAviCapture;      /* Capture AVI mode */	//kt
BOOL						bAviRun;          /* Capturing frames */	//kt
...
void avi_info_view(void)
{
	...
	{	/* INFO VIEW */
		...
		if (counter == 60)
		{
			...
			if (nAviFrameCount>0)
			{
				const unsigned int sec = ((double)nAviFrameCount / Machine->drv->screen[0].defstate.refresh);
		...
		if (counter < 50)
		{
			ui_draw_text2(buf, 0, 0, ARGB_RED);

			if (nAviFrameCount>0)
			{
				char bf[64];
				DWORD LapsedTime	= timeGetTime() - nAviStartTime;
				double tt = (double)(nAviFrameCountStart - nAviFrameCount) / Machine->drv->screen[0].defstate.refresh;
				double scale = (double)LapsedTime / 1000 / tt;
				DWORD RemainingTime = (DWORD)((double)nAviFrameCountStart / Machine->drv->screen[0].defstate.refresh * scale);
				LapsedTime		/= 1000;
				sprintf(bf, "TotalTime: %lu:%.2lu:%.2lu", RemainingTime/(60*60),RemainingTime/60%60,RemainingTime%60);
				ui_draw_text2(bf, 0, ui_get_line_height()*1, ARGB_RED);
				sprintf(bf, "Elapsed  : %lu:%.2lu:%.2lu", LapsedTime/(60*60),LapsedTime/60%60,LapsedTime%60);
				ui_draw_text2(bf, 0, ui_get_line_height()*2, ARGB_RED);
...
int avi_write_handler(mame_file *dummy, mame_bitmap *bitmap)
{
    if (bAviRun)
	{
		...
		if ((!mame_is_paused(Machine)) || (mame_is_paused(Machine) && get_single_step()))
		{
			AviAddBitmap(bitmap, NULL);
...
#ifdef USE_PSXPLUGIN
		if (window->primlist == NULL || (nUseExternalDraw && !mame_is_paused(Machine)))
#else
