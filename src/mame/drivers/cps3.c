...
static const struct game_keys2 keys_table2[] =
{
	...
	{ "jojobap",  { 0x23323ee3, 0x03021972 },0 },
...
MACHINE_START( cps3 )
{
	...
	if(!strcmp(Machine->gamedrv->name,"redearth") ||
		!strcmp(Machine->gamedrv->name,"warzard") ||
		!strcmp(Machine->gamedrv->name,"redeartn"))
	{
...
#endif /* KAILLERA */
		UINT32 *rom =  (UINT32*)memory_region ( REGION_USER1 );
		UINT32 dip = readinputport(2) & 0x0000000f;

#ifdef KAILLERA
//		if (kPlay) dip = 0; //Default
#endif /* KAILLERA */

		if (!dip) dip = default_dip;
		...
		//if (dip)
		{
			if(!strcmp(Machine->gamedrv->name,"redearth") ||
				...
				!strcmp(Machine->gamedrv->name,"redeartn"))
			{
				rom[0x1fed8/4] &= ~0x000000ff; // region hack
				rom[0x1fed8/4] |= dip;
				if (readinputport(2) & 0xf)
					rom[0x1fed8/4] |= 0x60;
			}
			else
			{
...
#ifdef MAME32PLUSPLUS
GAME_HACK( 1999, jojobap, jojoban,   cps3, cps3, jojoba,  ROT0,   "Capcom", "JoJo's Bizarre Adventure: Heritage for the Future / JoJo no Kimyouna Bouken: Miraie no Isan (Japan, 990913 / Ex Chars)", GAME_IMPERFECT_GRAPHICS )
#endif /* MAME32PLUSPLUS */
