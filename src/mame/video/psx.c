...
VIDEO_START( psx_type1 )
{
	...
	{ 
		psx_extgpu_init();
		if( _psxGPULib.bIsLoaded != FALSE ) {
			add_exit_callback(Machine, psx_video_exit);
			return;
		}
	}
#endif /* USE_PSXPLUGIN */
...
#ifdef USE_PSXPLUGIN
	if( _psxGPULib.bIsLoaded != FALSE ) {
		video_update_psx_extgpu( machine, screen, bitmap, cliprect );
...
#ifdef USE_PSXPLUGIN
	if( _psxGPULib.bIsLoaded != FALSE ) {
		psx_extgpu_w(offset, data, mem_mask);
		return;
	}
#endif /* USE_PSXPLUGIN */
	switch( offset )
...
#ifdef USE_PSXPLUGIN
	if( _psxGPULib.bIsLoaded != FALSE ) {
		return psx_extgpu_r(offset, mem_mask);
	}
#endif /* USE_PSXPLUGIN */
