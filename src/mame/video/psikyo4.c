...
{
#ifdef KAILLERA
	if (!strcmp(Machine->gamedrv->name,"hotgmck_k") ||
	    !strcmp(Machine->gamedrv->name,"hgkairak_k") ||
	    !strcmp(Machine->gamedrv->name,"hotgmck3_k") ||
	    !strcmp(Machine->gamedrv->name,"hotgm4ev_k") ||
	    !strcmp(Machine->gamedrv->name,"loderndf_k") ||
	    !strcmp(Machine->gamedrv->name,"loderndf_vs") ||
	    !strcmp(Machine->gamedrv->name,"hotdebut_k"))
	{
		int scr = 0x0000;
		if(kPlay) {
			int Kaillera_Pos = 0;
			...
			if (!strcmp(Machine->gamedrv->name,"loderndf_k") ||
			    !strcmp(Machine->gamedrv->name,"hotdebut_k"))
			{
			...
			if (readinputport(9) & 1) scr = 0x0000; /* change screens from false dip, is this ok? */
			else if (readinputport(9) & 2) scr = 0x2000;
		}

		fillbitmap(bitmap, Machine->pens[(scr==0x0000)?0x1000:0x1001], cliprect);
		draw_sprites(machine, bitmap, cliprect, scr);
	}
	else
	{
#endif /* KAILLERA */
...
{
#ifdef KAILLERA
	extern void init_psikyo4_driver(void);
#endif /* KAILLERA */
