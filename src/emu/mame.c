...
#ifdef MAME_AVI
	options_set_string(mame_options(), "avi_avi_filename", NULL, OPTION_PRIORITY_INI);
	options_set_string(mame_options(), "avi_wav_filename", NULL, OPTION_PRIORITY_INI);
#endif /* USE_IPS */
...
static void handle_save(running_machine *machine)
{
	...
	if (Machine->playback_sub_file)
	{
		...
		char namesub[2];
		_splitpath(astring_c(mame->saveload_pending_file), NULL, NULL, fname, NULL);
		...
		filerr = mame_fopen(SEARCHPATH_INPUTLOG, dstname, OPEN_FLAG_WRITE | OPEN_FLAG_CREATE | OPEN_FLAG_CREATE_PATHS, &file);
...
#ifdef KAILLERA
		if (!kPlay && !Machine->playback_sub_file)
		{
#endif /* KAILLERA */
...
static void handle_load(running_machine *machine)
{
...
#ifdef KAILLERA
	{
		...
		if (Machine->playback_sub_file)
		{
			...
			sprintf(path_new, "%s\\trctemp", path_old);
			options_set_string(mame_options(), SEARCHPATH_INPUTLOG, path_new, OPTION_PRIORITY_INI);
			filerr = mame_fopen(SEARCHPATH_INPUTLOG, dstname, OPEN_FLAG_READ, &file);
			options_set_string(mame_options(), SEARCHPATH_INPUTLOG, path_old, OPTION_PRIORITY_INI);
		...
		if (Machine->record_sub_file)
		{
			...
			if( i<65535 )
			{
				...
				extern void delete_file(const char *file_name);

				_splitpath(Trace_filename, NULL, NULL, fname, NULL);
				sprintf(filename, "%s\\%s-%s.sta", path_new, fname, namesub);
	...
	if (filerr == FILERR_NONE)
	{
		...
		if (state_save_load_begin(file) == 0)
		{
			...
			if (Machine->record_sub_file)
			{
				...
				if( i<65535 )
				{
					int fp = mame_ftell(Machine->record_sub_file);
					inpsub_header ih;
					...
					mame_fseek(Machine->record_sub_file, ((int)&ih.usestate[0]) - ((int)&ih) + perform_ui_statesave_file_fp++, SEEK_SET);
					mame_fwrite(Machine->record_sub_file, namesub, 1);
					mame_fseek(Machine->record_sub_file, fp, SEEK_SET);
