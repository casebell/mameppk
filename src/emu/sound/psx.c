...
#ifdef USE_PSXPLUGIN
	psx_extspu_start(sndindex, clock, config);
#endif /* USE_PSXPLUGIN */
	return chip;
}

#ifdef USE_PSXPLUGIN
static void psxspu_stop(void *token)
{
	if(_psxSPULib.bIsLoaded != FALSE) {
		psx_extspu_stop();
	}
...
#ifdef USE_PSXPLUGIN
	if(_psxSPULib.bIsLoaded != FALSE) {
		psx_extspu_w(offset, data, mem_mask);
	}
#endif /* USE_PSXPLUGIN */
	n_channel = offset / 4;
	if( n_channel < MAX_CHANNEL )
...
#ifdef USE_PSXPLUGIN
		case SNDINFO_PTR_STOP:							info->stop = psxspu_stop;				break;
#else /* USE_PSXPLUGIN */
